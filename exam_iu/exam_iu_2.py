...
class ExamIU2(QtCore.QState):
    def __init__(self, parent=None, server=None, form=None):
        ...
        self.stop_PCHV.addTransition(opc.pchv.updated,self.stop_PCHV)
        self.stop_PCHV.addTransition(self.stop_PCHV.done, self.disconnect_devices)
        ...
        self.stop_pchv1.addTransition(self.stop_pchv1.done, self.connect_pchv_reverse)
        self.stop_pchv1.addTransition(opc.pchv.updated, self.stop_pchv1)
        ...
        self.stop_pchv2.addTransition(self.stop_pchv2.done, self.connect_pchv2)
        self.stop_pchv2.addTransition(opc.pchv.updated, self.stop_pchv2)
...
class SetSpeed(QtCore.QState):
    def onEntry(self, QEvent):
        ...
        print("set speed", iu.speed[data.speed_idx])
...
class ExtractRes(QtCore.QState):
    def onEntry(self, QEvent):
        ...
        data_i=[v[0] for v in data.arr]
        data_a=[v[1] for v in data.arr]
        data_f=[v[2] for v in data.arr]

        max_i=max(data_i)
        half_index=data_i.index(max_i)

        data_i=data_i[:half_index]
        data_a=data_a[:half_index]
        data_f=data_f[:half_index]

        data.arr_average = []
        for a in range(0, 10):
            idx=len([v for v in data_a if v<=a+0.5])
            if idx>=len(data_a):
                idx=len(data_a)-1
            data_sa=data_a[idx-10:idx+10]
            data_si=data_i[idx-10:idx+10]
            data_sf=data_f[idx-10:idx+10]

            data_ai = list(zip(data_sa,data_si))
            data_af = list(zip(data_sa,data_sf))
            ai, bi = MNK(data_ai)
            af, bf = MNK(data_af)
            data.arr_average.append((ai * a + bi, a, af * a + bf))
            
        import logging
        logging.warning(data.arr)
        logging.warning(data.arr_average)


        if data.arr_average[1][0] == 0:
            data.note += 'Не удалось установить вал регулятора в позицию "1";'
            data.f1 = max([v[2] for v in data.arr if v[2]>0])
        else:
        ...
        if data.arr_average[9][0] == 0:
            ...
            data.f9 = min([v[2] for v in data.arr if v[2]>0])
        else:
